...
export const RegisterScreen = () => {
  ...
  const [signCheckbox, setSignCheckbox] = useState(false);
  ...
  if (showPass) {
    PassIcon = require('../../assets/images/icons/eyeball-icon.png');
  } else {
    PassIcon = require('../../assets/images/icons/eye-closed.png');
  }
  ...
  const UserRegister = () => {
    const checkPass = /^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{6,16}$/;
    ...
    } else if (!signCheckbox) {
      Dialog.show({
        type: ALERT_TYPE.WARNING,
        title: 'Alert!',
        textBody: 'Please check Sign up before going further.',
        button: 'OK',
        onPressButton: () => {
          Dialog.hide();
        },
            ...
            <View style={IntroStyle.CheckboxContainer}>
              <View style={IntroStyle.CheckboxFlex}>
                <CheckBox
                  ...
                  onValueChange={newValue => setSignCheckbox(newValue)}
                />
