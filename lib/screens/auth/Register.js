...
export const RegisterScreen = () => {
  ...
  const UserRegister = () => {
    const strongPassword = /^\(?\d{3}\)?-? *\d{3}-? *-?\d{4}$/;
    ...
    if (!firstName) {
      ...
      Dialog.show({
        ...
        title: 'Warning!',
        textBody:
          'Please add a strong password! The password should have Minimum eight characters, at least one letter, one number and one special character:',
        button: 'OK',
      });
    } else {
      setLoading(true);
